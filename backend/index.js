...
const MEDICATION_SCHEMA = {
  ...
  "type": "object",
  "required": ["raw_ocr_reference", "medication_name", "pictogram_categories"],
  "properties": {
...
app.post('/api/ocr/extract', upload.single('file'), async (req, res) => {
  try {
    ...
    // 2. Parse text into your specific Schema using Gemini
    const structuredData = await parseMedicationLabel(extractedText);

    // 3. Return the FINAL structured data to the mobile app
